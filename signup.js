...
function enterDashboard() {
    ...
    // Get selected company data and store in localStorage for dashboard
    const selectedCompanyData = document.getElementById('selectedCompany').value;
    const companyTicker = document.getElementById('companyTicker').value;
    
    if (selectedCompanyData && companyTicker) {
        localStorage.setItem('selectedCompany', selectedCompanyData);
        localStorage.setItem('companyTicker', companyTicker);
        console.log('Stored company data for dashboard:', { selectedCompanyData, companyTicker });
    }
        ...
        <div class="company-search-result" onclick="selectCompany('${company.symbol}', '${encodeURIComponent(JSON.stringify(company))}')">
            <div class="company-search-logo">
                <img src="https://financialmodelingprep.com/image-stock/${company.symbol}.png" 
                     ...
                     style="display: block; width: 32px; height: 32px; border-radius: 4px;">
                <div class="company-logo-placeholder" style="display: none; width: 32px; height: 32px; border-radius: 4px; background: #e5e7eb; align-items: center; justify-content: center; font-size: 12px; font-weight: bold; color: #6b7280;">${company.symbol}</div>
            </div>
